...
//        if !m.dm.goalOperatorLearning  {
            chunk.assocs[goalName] = Assocs(name: goalName, sji: m.dm.defaultOperatorAssoc, unknown: 0)
//        }
        chunk.assocs[chunk.name] = Assocs(name: chunk.name, sji: m.dm.defaultOperatorSelfAssoc, unknown: 0)
        for (_,ch) in m.dm.chunks {
            if ch.type == "operator" && ch.assocs[goalName] != nil {
                chunk.assocs[ch.name] = Assocs(name: ch.name, sji: m.dm.defaultInterOperatorAssoc, unknown: 0)
                ch.assocs[chunk.name] = Assocs(name: chunk.name, sji: m.dm.defaultInterOperatorAssoc, unknown: 0)
            }
        }
...
//        println("Adding operator\n\(chunk)")
    ...
    func parseSjis() -> Bool {
        ...
        while !scanner.scanString("}", intoString: nil) {
            ...
            m.dm.chunks[iChunkName!]!.assocs[jChunkName!] = Assocs(name: jChunkName!, sji: assocValue!, unknown: 0)
            if !scanner.scanString(")", intoString: nil) {
                m.addToTraceField("Missing ')' in Sji definition.")
            }
